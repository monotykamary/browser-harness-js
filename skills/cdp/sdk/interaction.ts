...
export type InteractionCandidate = { id: string; role: string; label: string; operations: string[]; value?: string; checked?: boolean };
...
export class InteractionController {
  ...
  constructor(session: InteractionSession, options: { allowedOrigins: string[] }) {
    if (!options || !Array.isArray(options.allowedOrigins) || options.allowedOrigins.length < 1 || options.allowedOrigins.length > 32 ||
        options.allowedOrigins.some(origin => typeof origin !== 'string' || origin.length > 2048)) {
    ...
    this.unsubscribe = session.onEvent?.((method, params, sid) => {
      if (method === 'Session.connectionChanged') this.invalidate();
      else if (method === 'Target.detachedFromTarget') {
        if (params?.sessionId) this.invalidate({ sessionId: params.sessionId });
      } else if (['Page.frameNavigated', 'Page.navigatedWithinDocument', 'Runtime.executionContextsCleared',
        'DOM.documentUpdated', 'Inspector.detached', 'Inspector.targetCrashed'].includes(method)) {
  ...
  async observe(input: { scope: InteractionScope; maxElements?: number }, options: Options = {}): Promise<InteractionObservation> {
    const sid = this.scopeId(input.scope);
    const max = input.maxElements ?? 64;
    if (!Number.isInteger(max) || max < 1 || max > 128) throw new Error('maxElements must be an integer from 1 to 128');
    const signal = this.signal(options);
    return this.serial(sid, () => this.snapshot(sid, max, signal), signal);
  }

  async act(input: { scope: InteractionScope; observationId: string; action: { targetId: string; operation: string; text?: string } }, options: Options = {}): Promise<InteractionReceipt> {
    const sid = this.scopeId(input.scope);
    ...
    if (typeof observationId !== 'string' || observationId.length > 256 ||
        typeof input.action?.targetId !== 'string' || input.action.targetId.length > 256) return { status: 'stale', reason: 'unknown_target' };
    const existing = this.records.get(sid);
    if (!existing || existing.consumed || existing.observation.observationId !== observationId || !this.current(sid, existing)) {
      return { status: 'stale', reason: 'observation_expired' };
    }
    const unsupported = !['click', 'type'].includes(input.action.operation) ||
      (input.action.operation === 'type' && (typeof input.action.text !== 'string' || input.action.text.length > 4096));
    // Do not retain arbitrary caller payloads (including unsupported oversized text) in queues.
    const action = { targetId: input.action.targetId, operation: unsupported ? '' : input.action.operation,
      text: !unsupported && input.action.operation === 'type' ? input.action.text : undefined };
    let dispatched = false;
    return this.serial<InteractionReceipt>(sid, async () => {
      const record = this.records.get(sid);
      try {
        if (signal.aborted) return { status: 'blocked', reason: 'cancelled' };
        if (!record || record.consumed || record.observation.observationId !== observationId || !this.current(sid, record)) {
          return { status: 'stale', reason: 'observation_expired' };
        }
        record.consumed = true;
        const index = record.targets.get(action.targetId);
        if (index === undefined) return { status: 'stale', reason: 'unknown_target' };
        if (!record.observation.candidates[index]?.operations.includes(action.operation) ||
            (action.operation === 'type' && (typeof action.text !== 'string' || action.text.length > 4096))) {
        ...
        if (frame.key !== record.frame || !this.current(sid, record)) return { status: 'stale', reason: 'navigation_or_connection_changed' };
        ...
        const result = await this.call(sid, 'Runtime.callFunctionOn', {
          ...
          functionDeclaration: 'function(index, operation, text) { return this.act(index, operation, text); }',
          arguments: [{ value: index }, { value: action.operation }, { value: action.text ?? '' }],
          returnByValue: true,
        }, record.generation, signal);
        const receipt = this.value(result) as InteractionReceipt;
        ...
        if (!this.current(sid, record) || signal.aborted) return { status: 'outcome_unknown', reason: 'changed_during_dispatch' };
        return receipt;
      } catch {
        return { status: dispatched ? 'outcome_unknown' : signal.aborted ? 'blocked' : 'stale', reason: dispatched ? 'dispatch_unconfirmed' : signal.aborted ? 'cancelled' : 'revalidation_failed' };
      }
    }, signal).catch(() => ({ status: dispatched ? 'outcome_unknown' : 'blocked', reason: signal.aborted ? 'cancelled' : 'capacity_exceeded' }));
  }

  async waitForChange(input: { scope: InteractionScope; revision: string; timeoutMs?: number }, options: Options = {}): Promise<{ changed: boolean; observation: InteractionObservation }> {
    const sid = this.scopeId(input.scope);
    const timeout = input.timeoutMs ?? 5000;
    if (!Number.isFinite(timeout) || timeout < 0 || timeout > 60000) throw new Error('timeoutMs must be from 0 to 60000');
    const signal = this.signal(options);
    signal.throwIfAborted();
    if (typeof input.revision !== 'string' || input.revision.length > 256) throw new Error('revision must be at most 256 characters');
    if (this.waiters >= MAX_PENDING) throw new Error('Interaction wait capacity exceeded');
    this.waiters++;
    try {
      const max = this.records.get(sid)?.maxElements ?? 64;
      ...
      for (;;) {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          const timer = setTimeout(() => { signal.removeEventListener('abort', abort); resolve(); }, Math.min(500, end - Date.now()));
  ...
  private scopeId(scope: InteractionScope): string {
    if (!scope || typeof scope.sessionId !== 'string' || !scope.sessionId.trim() || scope.sessionId.length > 256) throw new Error('Explicit nonempty scope.sessionId of at most 256 characters is required');
  ...
  private allowed(url: string): boolean {
    try { return this.origins.includes(new URL(url).origin); } catch { return false; }
  }
  private current(sid: string, record: RecordState): boolean {
    return !this.lifetime.signal.aborted && this.records.get(sid) === record &&
      this.epochs.get(sid) === record.epoch && record.generation === this.session.getConnectionGeneration?.() &&
      this.session.isConnected?.() !== false;
  }
  private async call(sid: string, method: string, params: unknown, generation: number | undefined, signal: AbortSignal): Promise<any> {
    signal.throwIfAborted();
    if (this.session.isConnected?.() === false || generation !== this.session.getConnectionGeneration?.()) throw new Error('Connection changed');
  ...
  private value(result: any): any {
    if (result?.exceptionDetails || !result?.result || !('value' in result.result)) throw new Error('Guard context unavailable');
  ...
  private async frame(sid: string, generation: number | undefined, signal: AbortSignal) {
    ...
    return { id: frame.id as string, url: frame.url as string, key: JSON.stringify([frame.id, frame.loaderId, frame.url]) };
  }
  ...
  private async snapshot(sid: string, max: number, signal: AbortSignal): Promise<InteractionObservation> {
    ...
    try {
      ...
      const world = await this.call(sid, 'Page.createIsolatedWorld', { frameId: frame.id, worldName: 'browser-harness-guard' }, generation, signal);
      const evaluated = await this.call(sid, 'Runtime.evaluate', {
        expression: `(${projection})(${JSON.stringify(this.origins)}, ${max})`,
        contextId: world.executionContextId,
        objectGroup: 'browser-harness-guard', awaitPromise: true,
      }, generation, signal);
      ...
      const data = this.value(await this.call(sid, 'Runtime.callFunctionOn', {
        objectId, functionDeclaration: 'function() { return this.snapshot; }', returnByValue: true,
      }, generation, signal));
      if (data.denied || !this.allowed(data.url)) throw new Error('origin_denied');
      const after = await this.frame(sid, generation, signal);
      if (after.key !== frame.key || this.epochs.get(sid) !== epoch || generation !== this.session.getConnectionGeneration?.()) throw new Error('Observation changed; reobserve');
      ...
      const observation: InteractionObservation = {
        scope: { sessionId: sid }, observationId: randomUUID(),
        revision: createHash('sha256').update(JSON.stringify([generation, frame.key, data])).digest('hex'),
        candidates, url: data.url, title: data.title, truncated: data.truncated, truncation: data.truncation,
      };
      this.records.set(sid, { objectId, epoch, generation, frame: frame.key, observation: structuredClone(observation), targets, consumed: false, maxElements: max });
      return observation;
    } catch (error) {
      if (objectId) this.release(sid, objectId, generation);
...
// Bounded light-DOM native controls only. No AX completeness or trusted-input claim.
const projection = String.raw`async function(allowedOrigins, maxElements) {
  const doc = document;
  ...
  const secure = el => {
    ...
    if (autocomplete.length > 2048) { textTruncated = true; return true; }
    if (autocomplete.toLowerCase().split(/\s+/).some(token => token.includes('password') || token.startsWith('cc-') || token === 'one-time-code')) return true;
    ...
    if (el.labels && el.labels.length > 8) { textTruncated = true; return true; }
    names.push(autocomplete, el.textContent || '', ...Array.from(el.labels || []).map(x => x.textContent || ''));
    for (const name of names) {
      if (name.length > 2048) { textTruncated = true; return true; }
  ...
  const semantics = el => {
    ...
    const editable = (tag === 'INPUT' && ['text', 'search'].includes(el.type)) || tag === 'TEXTAREA';
    const clickable = tag === 'BUTTON' || (tag === 'A' && el.hasAttribute('href')) || (tag === 'INPUT' && ['button', 'submit', 'reset', 'checkbox', 'radio'].includes(el.type));
    if (!editable && !clickable) return null;
    // Unsupported ARIA overrides are not guessed at.
    const role = tag === 'A' ? 'link' : editable ? (el.type === 'search' ? 'searchbox' : 'textbox') : ['checkbox', 'radio'].includes(el.type) ? el.type : 'button';
    if (el.hasAttribute('aria-labelledby') || (el.hasAttribute('role') && el.getAttribute('role') !== role)) return null;
    if ((el.textContent || '').length > 2048 || (el.labels && (el.labels.length > 8 || Array.from(el.labels).some(x => (x.textContent || '').length > 2048)))) { textTruncated = true; return null; }
    const labels = el.labels ? Array.from(el.labels).map(x => x.textContent || '').join(' ') : '';
    const label = el.getAttribute('aria-label') || labels || el.getAttribute('title') || (editable ? el.getAttribute('placeholder') : el.textContent) || '';
    if (editable && el.value.length > 4096) { textTruncated = true; return null; }
    return { role, label: bounded(label.trim(), 256), operations: editable ? (el.readOnly ? [] : ['type']) : ['click'],
      ...(editable ? { value: el.value } : {}), ...(['checkbox', 'radio'].includes(role) ? { checked: !!el.checked } : {}) };
  };
  const state = el => {
    ...
    for (let p = el; p; p = p.parentElement) {
      ...
      if (style.display === 'none' || style.visibility !== 'visible' || Number(style.opacity) === 0 || p.hidden || p.inert || p.getAttribute('aria-hidden') === 'true') visible = false;
    }
    ...
    if (el.attributes.length > 64) { textTruncated = true; return null; }
    const attrs = Array.from(el.attributes);
    if (attrs.some(a => a.name.length > 256 || a.value.length > 1024) || ('value' in el && el.value.length > 4096)) { textTruncated = true; return null; }
    const parts = [[semantic.role, semantic.label, semantic.operations], attrs.map(a => [a.name, a.value]), el.textContent,
      el.labels ? Array.from(el.labels).map(x => x.textContent) : [], 'value' in el ? el.value : null, 'checked' in el ? el.checked : null, rect.x, rect.y, rect.width, rect.height];
    // Exact local identity, never a sliced fingerprint. Bound even the pre-serialization input.
    if (attrs.reduce((n, a) => n + a.name.length + a.value.length, 0) > 8192) { textTruncated = true; return null; }
    const fingerprint = JSON.stringify(parts);
    if (fingerprint.length > 8192) { textTruncated = true; return null; }
    return { semantic, visible, clear, enabled, fingerprint };
  };
  const walker = doc.createTreeWalker(doc.documentElement, NodeFilter.SHOW_ELEMENT);
  let scanned = 0, elementsTruncated = false, node;
  while (scanned < 4096 && (node = walker.nextNode())) {
    scanned++;
    const s = state(node);
    if (!s || !s.visible || !s.clear || !s.enabled) continue;
    if (refs.length === maxElements) { elementsTruncated = true; break; }
    refs.push({ element: node, initial: s });
  }
  const title = bounded(doc.title, 512);
  const safeUrl = bounded(url, 2048);
  const truncation = { elements: elementsTruncated, scan: scanned === 4096, text: textTruncated };
  ...
  const snapshot = { url: safeUrl, title, candidates: refs.map(ref => ref.initial.semantic), revisionState, truncated: Object.values(truncation).some(Boolean), truncation };
  let consumed = false;
  return { snapshot, act(index, operation, text) {
    ...
    if (operation === 'click') {
      ...
      HTMLElement.prototype.click.call(ref.element);
    } else if (operation === 'type' && typeof text === 'string' && text.length <= 4096) {
      const proto = ref.element.tagName === 'TEXTAREA' ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype;
      Object.getOwnPropertyDescriptor(proto, 'value').set.call(ref.element, text);
      ref.element.dispatchEvent(new Event('input', { bubbles: true, composed: true }));
    } else return { status: 'blocked', reason: 'unsupported_action' };
